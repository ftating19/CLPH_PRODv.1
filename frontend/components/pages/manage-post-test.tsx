...
import { BookOpen, Clock, Plus, Search, FileText, Users, XCircle, Check, ChevronsUpDown, Eye, Trash2 } from "lucide-react"
...
export default function ManagePostTest() {
  ...
  const filteredTemplates = templates.filter(template => {
    ...
    const matchesSubject = selectedSubjectFilter === "all" || 
                          template.subject_id?.toString() === selectedSubjectFilter
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create reusable post-test templates and assign them to multiple students
          </p>
        </div>
        <Button onClick={() => {
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-sm">
                              <span className="text-muted-foreground">
                                Status: <Badge variant={assignment.status === 'completed' ? 'default' : 'secondary'}>
                                  {assignment.status}
                                </Badge>
                              </span>
      ...
      <Dialog open={showCreateTemplateDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingTemplateId ? 'Edit Template' : 'Create Post-Test Template'}</DialogTitle>
            <DialogDescription>
              {editingTemplateId 
                ? 'Update the template details and questions below.'
                : 'Create a reusable post-test template that can be assigned to multiple students.'
              }
            </DialogDescription>
          </DialogHeader>
