...
export default function FeedbackRating() {
  ...
  useEffect(() => {
    const loadUserFeedback = async () => {
      ...
      try {
        ...
        await fetch('http://localhost:4000/api/system-feedback/init-table')
        
        // Then load user's feedback history
        const response = await fetch(`http://localhost:4000/api/system-feedback/user/${currentUser.user_id}`)
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4000/api/system-feedback', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        const historyResponse = await fetch(`http://localhost:4000/api/system-feedback/user/${currentUser.user_id}`)
